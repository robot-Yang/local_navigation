...
# k5 = 4
# default value for pre-configuration
k1, k2, k3, k4, k5 = 2.48, 0.91, 1.59, 1.75, 1.46
...
# pr2 = 2.5
# classification point for center of pressure ox(calibration needed)
pl2, pl1, pr1, pr2 = -0.97, -0.2, 0.2, 1.17
...
def log(a, b, c, d, e, ox, Command_DAC0, Command_DAC1, filename,level,fmt='%(created).6f %(message)s'):
    ...
    logger.info("%s %s %s %s %s %s %s %s",a, b, c, d, e, ox, Command_DAC0, Command_DAC1)
...
def execution(a, b, c, d, e, ox):
    ...
    # turn an angle
        ...
        # continue
    # turn around
    elif (ox <= pl2 and e <= treshold and b >= treshold):
        # print("around")
        ...
        # continue
    elif (ox >= pr2 and a<=treshold and d >= treshold):
        # print("around")
        conv.SET_DAC0(2500, conv.data_format.voltage)
...
def main():
    ...
    a, b, c, d, e, ox, = a.zfill(6),b.zfill(6),c.zfill(6),d.zfill(6),e.zfill(6),ox.zfill(6),
    # log(a, b, c, d, e, ox, Command_DAC0, Command_DAC1, 'info.log', level='info')

...
order = 3
fs = 7      # progrm frequency, detected by float(counter1) / float(end - start)in the last line
cutoff = 3  # desired cutoff frequency of the filter, Hz
counter = 0  # for trigger butterworth filter
counter1 = 0 # for calculate frequency
b, a = butter_lowpass(cutoff, fs, order)
A_z = np.zeros(max(len(a), len(b)) - 1, dtype=np.float)
B_z = np.zeros(max(len(a), len(b)) - 1, dtype=np.float)
C_z = np.zeros(max(len(a), len(b)) - 1, dtype=np.float)
D_z = np.zeros(max(len(a), len(b)) - 1, dtype=np.float)
E_z = np.zeros(max(len(a), len(b)) - 1, dtype=np.float)
...
def butterworth():
    ...
    global A, B, C, D, E
    global A_z, B_z, C_z, D_z, E_z
    A1, A_z = lfilter(b, a, A, zi=A_z)
    B1, B_z = lfilter(b, a, B, zi=B_z)
    C1, C_z = lfilter(b, a, C, zi=C_z)
    D1, D_z = lfilter(b, a, D, zi=D_z)
    E1, E_z = lfilter(b, a, E, zi=E_z)

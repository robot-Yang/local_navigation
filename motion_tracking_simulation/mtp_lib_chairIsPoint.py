...
class chen():
    ...
    def SpeedToGo(self):
        ...
        if self.rho < 0.3:
            ...
            self.w = self.Kp_beta * self.beta + self.K4 * (self.spiralAngle + self.bearingAngle)
            ...
            # self.w = self.Kp_alpha * self.alpha + self.Kp_beta * self.beta - self.K4 * ((self.theta - self.AngleCenter + np.pi) % (2 * np.pi) - np.pi)
            # self.w = self.Kp_alpha * self.alpha + self.Kp_beta * self.beta - self.K4 * self.rho
            # self.w = self.Kp_beta * (self.Kp_alpha * self.alpha + self.beta) * self.rho
            # self.w = (self.Kp_alpha * self.alpha + self.Kp_beta * self.beta) + self.K4 * self.bearingAngle #* self.rho
            # self.w = (self.Kp_alpha * self.alpha + self.Kp_beta * self.beta)
            # self.w = (self.Kp_alpha * self.alpha + self.Kp_beta * self.beta) * self.rho
            # self.w = self.K4*(self.spiralAngle + self.bearingAngle)
            # self.w = (self.Kp_alpha * self.alpha + self.Kp_beta * self.beta) + self.K4 * (self.spiralAngle + self.bearingAngle)
            self.w = self.Kp_beta * self.beta + self.K4 * (self.spiralAngle + self.bearingAngle)
            # self.w = self.Kp_alpha * self.alpha + self.K4 * (self.firstspiralAngle + self.bearingAngle)
    ...
    def collect(self, L_x_chair, L_y_chair, R_x_chair, R_y_chair):
        ...
        self.FOV.append(abs(self.bearingAngle))  #field of view
    ...
    def move_to_pose(self, x_start, y_start, theta_start, x_goal, y_goal, theta_goal, L_x_chair, L_y_chair, R_x_chair, R_y_chair, F_x_chair, F_y_chair):
        ...
        while (self.rho > 0.01) and count < 150:
            count += 1
            # print("count =", count)

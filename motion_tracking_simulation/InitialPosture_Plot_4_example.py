...
# @Last Modified time: 2020-12-26 23:56:43
...
def initializePoint():
    for rho in np.linspace(0, 2.0, samRho):
        phi_max = rho / k4**2
        for phi in np.linspace(-phi_max, phi_max, samPhi):
            alpha_max = k6*phi + (1- abs(phi) / phi_max) * (alpha_bar*np.pi/180) * k5 * rho # k6*phi is the centerline of alpha
            alpha_min = k6*phi - (1- abs(phi) / phi_max) * (alpha_bar*np.pi/180) * k5 * rho
            # alpha_max = (1- abs(phi) / (rho**3 / k4**2)) * (alpha_bar*np.pi/180)
            Alpha = np.linspace(alpha_min, alpha_max, samAlpha)
            Alpha = Alpha.tolist()
...
def TwoDspace(x_start,y_start,theta_start,color):
    ...
    if color == 'black':
        plt.quiver(x_start, y_start, np.cos(theta_start), np.sin(theta_start), color = 'k', scale =35)
    else:

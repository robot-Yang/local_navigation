...
# @Last Modified time: 2020-12-26 21:46:57

'''
replot/check trajectory & a* of selected data(simulation_result_1.csv)
plot for example which is shown in the paper, increase the readability
'''
...
class chen():
    ...
    def Angle(self):
        ...
        self.beta = theta_goal - np.arctan2(self.y_diff, self.x_diff)
    ...
    def collectPlot(self):
        ...
        plt.ylabel(r'$\alpha_*$''[degree]', fontsize=16)
        ...
        # plt.figure('AlphaStar_LR-Rho')
        # plt.plot(Rho, AlphaStar_LR, 'g-')
        # plt.xlabel("Rho")
        # plt.ylabel("AlphaStar_LR[degree]")
        # plt.xlim(-0.2, 1.5)
        # plt.ylim(-60, 60)
        ...
        # if max(Abs_AlphaStar) < alpha_bar and judge:
            ...
            ax.set_aspect(1)
            chair_info(Chair)
            plt.arrow(x_start, y_start, 0.05*np.cos(theta_start),
                      0.05*np.sin(theta_start), color='r', width=0.02)
            plt.arrow(x_goal, y_goal, 0.05*np.cos(theta_goal),
                      0.05*np.sin(theta_goal), color='g', width=0.02)
            plt.scatter(chair_bottom_x, chair_bottom_y, s=50, c='r', marker="x")
            plt.scatter(x_goal, y_goal, s=60, c='k', marker=".", zorder=16) # 
            plt.scatter(x_start, y_start, s=60, c='k', marker=".", zorder=16)
            plt.xticks(fontsize=16) 
            plt.yticks(fontsize=16)
            plt.xlim(-2, 2)
            plt.ylim(-1.5, 1)
            ...
            plt.figure('Left_AlphaStar-Rho')
            plt.plot(AlphaStar, Rho, 'g-')
            plt.ylabel(r'$\rho$''[meter]', fontsize=16)
...
samPhi = 12 # sampling number/step of phi
Rhoo = []
Alphaa = []
Phii = []
rhoo = np.linspace(0.2, 2, samRho)

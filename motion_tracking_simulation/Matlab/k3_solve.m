...
% k3 = k1;
% % f(k4) = (k4^2*sin((pi*alphabar)/180)^4 - 2*k1*k2 - 4*k1*k3 + k1^2 + k2^2 - 2*k1*k4*sin((pi*alphabar)/180)^2 + 2*k2*k4*sin((pi*alphabar)/180)^2)^(1/2)/2;
% f(k4) = k1^2 - 2*k1*k2 - 2*k1*k4 - 4*k3*k1 + k2^2 + 2*k2*k4 + k4^2;
% x4 = solve(f(k4),k4);
% x4
% x5 = (x4*sin((pi*alphabar)/180)^2)/2;
% x5 = simplify(x5);
% x5
% k1 = 0.5; k2 = 2:
% x1 = subs(x0,{k1, k2, alphabar},{0.5,2,40});

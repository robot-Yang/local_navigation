...
# @Last Modified time: 2020-12-26 21:27:22
...
def plotPoints():
    ...
    ax = plt.gca()
    ax.set_aspect(1)
    plt.quiver(Left_initialX, Left_initialY, np.cos(Left_initialPosture), np.sin(Left_initialPosture), scale =25)
    plt.arrow(x_goal, y_goal, 0.15*np.cos(theta_goal),
              0.15*np.sin(theta_goal), color='g', width=0.03)
    plt.scatter(chair_bottom_x, chair_bottom_y, s=50, c='r', marker="x")
    plt.scatter(x_goal, y_goal, s=60, c='k', marker=".", zorder=30) # 
...
class DockingSim():
    ...
    def PlotAll(self):
        ...
        ax.set_aspect(1)
        plt.arrow(x_start, y_start, 0.15*np.cos(theta_start),
                  0.15*np.sin(theta_start), color='r', width=0.02)
        plt.arrow(x_goal, y_goal, 0.15*np.cos(theta_goal),
                  0.15*np.sin(theta_goal), color='g', width=0.02)
        plt.scatter(chair_bottom_x, chair_bottom_y, s=50, c='r', marker="x")
        self.plot_vehicle()
